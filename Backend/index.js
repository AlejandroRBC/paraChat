...
//crear el usuario para las ventas rapidas
db.run(`INSERT OR IGNORE INTO cliente (cod_cli, nombre, ci_nit, descuento, estado) VALUES (0, 'S/N', '00000', 0, 'activo')`);
...
// ENDPOINTS PARA VENTAS (SIMPLIFICADO)
// ----------------------

// POST crear venta
app.post("/api/ventas", (req, res) => {
  const { total, metodo_pago, id_cliente, items } = req.body;
  
  // Validación básica
  if (!items || items.length === 0) {
    res.status(400).json({ error: 'No hay items en la venta' });
    return;
  }
  ...
  db.serialize(() => {
    ...
    db.run(sqlVenta, [total, metodo_pago, id_cliente || null], function(err) {
      if (err) {
        res.status(400).json({ error: err.message });
        return;
      }
      ...
      // 2. Insertar los detalles de venta y actualizar stock
      items.forEach((item) => {
        ...
        db.run(sqlDetalle, [idVenta, item.id_producto, item.cantidad, subtotal], function(err) {
          if (err) {
            res.status(400).json({ error: err.message });
            return;
          }
          
          // Actualizar stock del producto (sin validación de stock)
          ...
          db.run(sqlActualizarStock, [item.cantidad, item.id_producto], function(err) {
            if (err) {
              res.status(400).json({ error: err.message });
              return;
            }
            ...
            if (detallesInsertados === items.length) {
              // Obtener la venta completa con detalles
              const sqlVentaCompleta = `
                SELECT 
                  v.*,
              ...
              db.get(sqlVentaCompleta, [idVenta], (err, venta) => {
                if (err) {
                  res.status(400).json({ error: err.message });
                  return;
                }
                ...
                db.all(sqlDetalles, [idVenta], (err, detalles) => {
                  if (err) {
                    res.status(400).json({ error: err.message });
                    return;
                  }
...
app.get("/api/ventas/:id", (req, res) => {
  ...
  db.get(sqlVenta, [req.params.id], (err, venta) => {
    if (err) {
      res.status(400).json({ error: err.message });
      return;
    }
    ...
    db.all(sqlDetalles, [req.params.id], (err, detalles) => {
      if (err) {
        res.status(400).json({ error: err.message });
        return;
      }
...
// GET detalle de ventas por rango de fechas
app.get("/api/ventas-reporte", (req, res) => {
